...
@endsection
@section('content')
    <form action="{{ route('transactionLogs.update', ['transactionLog' => $transactionLog->id, 'id' => $id]) }}"
        method="POST">
        @csrf
        @method('put')
        ...
        <div class="form-group">
            ...
            <select name="currency" id="currency" class="form-control">
                ...
                <option value="egp" @if ($transactionLog->currency == 'egp') selected @endif>جنيه مصري</option>
                <option value="try" @if ($transactionLog->currency == 'try') selected @endif>ليرة تركية</option>
                <option value="other" @if ($transactionLog->currency == 'other') selected @endif>أخرى</option>
            </select>
            @error('currency')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        ...
        <div class="form-group">
            ...
            <input type="date" name="request_date" id="request_date" class="form-control"
                value="{{ $transactionLog->request_date }}">
            @error('request_date')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
